...
def process_image(event, context):
    ...
    yolo_max_labels = config["yolo_max_labels"]
    yolo_min_conf = float(config["yolo_min_conf"])
    ...
    for record in event['Records']:
        ...
        try:
            results = model(
                ...
                MaxLabels=yolo_max_labels,
                MinConfidence=yolo_min_conf
            )
            results.print()
        ...
        #Iterate on yolo labels. Enrich and prep them for storage in DynamoDB
        ...
        for item in result:
            ...
            if (lbl.upper() in (label.upper() for label in label_watch_list)
                ...
                labels_on_watch_list.append(deepcopy(item))

            #Convert from float to decimal for DynamoDB
            print("confidence type", type(item['confidence']))
            item['confidence'] = Decimal(conf)
        ...
        # Send out notification(s) using sns
